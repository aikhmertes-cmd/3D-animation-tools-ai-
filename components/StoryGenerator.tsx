...
const StoryGenerator: React.FC = () => {
    ...
    const handleAiSuggestCharacters = useCallback(async () => {
        ...
        if (!context.trim()) {
            setError('Please provide a Story Topic or Script before generating characters.');
            return;
        }
        setIsGeneratingCharacters(true);
        setError(null);
        try {
            const generated = await generateCharacters(context, aiCharacterCount);
    ...
    }, [topic, pastedStory, mode, aiCharacterCount]);
                        ...
                        <div className="space-y-4 animate-fade-in">
                             <div className="p-4 rounded-lg bg-gray-900/50 border border-gray-700 space-y-3">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={handleAiSuggestCharacters} disabled={isGeneratingCharacters || !(topic.trim() || pastedStory.trim())} className="flex-1 flex items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-teal-500 to-cyan-500 rounded-lg shadow-md hover:from-teal-600 hover:to-cyan-600 transition disabled:opacity-50">
                                      {isGeneratingCharacters ? 'Generating...' : 'Generate with AI'}
                                    </button>
                                </div>
                                 <p className="text-xs text-gray-500">Requires a story topic or pasted script in the "Story Settings" tab.</p>
                            </div>
                            {characters.map((char, index) => (
                                <div key={char.id} className="p-4 rounded-lg bg-gray-800 border border-gray-700 relative space-y-3">
                                    ...
                                    <button onClick={() => removeCharacter(char.id)} disabled={characters.length <= 1} className="absolute top-2 right-2 p-1.5 text-gray-500 hover:text-red-400 disabled:opacity-30 transition">
                                        <TrashIcon />
                                    </button>
                                </div>
                            ))}
