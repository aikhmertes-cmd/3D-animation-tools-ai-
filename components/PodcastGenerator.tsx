...
// --- Helper Components ---
const Spinner: React.FC<{className?: string}> = ({className = "-ml-1 mr-3 h-5 w-5"}) => (
                ...
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 space-y-6 h-fit">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {[2, 3, 4, 5, 6, 7, 8, 9].map(min => (
                                <button
                                    key={min}
                                    onClick={() => setDurationInMinutes(min)}
                                    disabled={isLoading}
                                    className={`px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 disabled:opacity-50 ${durationInMinutes === min ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                                >
                                    {min}
                                </button>
                            ))}
